...
def bootstrap_owner():
    email=os.getenv('OWNER_EMAIL','').lower().strip();password=os.getenv('OWNER_PASSWORD','')
    if not email or not password:return
    db=SessionLocal()
    try:
        if not db.query(User).filter(User.email==email).first():
            db.add(User(email=email,display_name='Owner',password_hash=hash_password(password),role='owner'));db.commit()
    finally:db.close()
bootstrap_owner()
...
def home(req:Request,db:Session=Depends(get_db)):
    return templates.TemplateResponse('home.html',{'request':req,'user':current_user(req,db),'csrf':csrf(req),'app_name':APP_NAME})
...
def login_page(req:Request):return templates.TemplateResponse('login.html',{'request':req,'csrf':csrf(req),'app_name':APP_NAME,'google_enabled':bool(GOOGLE_CLIENT_ID),'apple_enabled':bool(APPLE_CLIENT_ID)})
@app.post('/login')
