...
def auth_apple(req:Request):
    if not APPLE_CLIENT_ID:raise HTTPException(503,'Apple sign-in is not configured yet')
    state=secrets.token_urlsafe(24);req.session['apple_state']=state
    return RedirectResponse('https://appleid.apple.com/auth/authorize?'+urlencode({'client_id':APPLE_CLIENT_ID,'redirect_uri':public_base_url(req)+'/api/auth/apple/callback','response_type':'code','response_mode':'form_post','scope':'name email','state':state}))
